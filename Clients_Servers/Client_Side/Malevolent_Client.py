...
def connect_to_server(host, port, num_attempts=5, delay=1):
    ...
    for attempt in range(num_attempts):
        try:
            ...
            with socket.create_connection((host, port), timeout=5) as s:
                print(f"Malevolent Client: trying to connect to the server {host}:{port}")
                print(f"🚨 Malevolent Client: Warning: No data sent. Waiting for connection to close.")
        ...
        # Handing connection timeout or refusal and retries after a delay
            ...
            print(f"Malevolent Client: Attempt {attempt + 1}/{num_attempts} failed: {e}")
            time.sleep(delay)

        if attempt == num_attempts - 1:
            print("Malevolent Client: Connection attempts exhausted.")
