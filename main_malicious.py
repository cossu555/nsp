...
while True:
    try:
        server_port = int(input("Write the port you want to use as the server port: "))
        if server_port <= 0 or server_port > 65535:
            raise ValueError("Port number must be between 1 and 65535.")
        break
    except ValueError as e:
        print(f"Invalid input: {e}. Please enter a valid port number.")          # Standard HTTPS port for secure communication
...
time.sleep(10)  # Allow some time for the connection to establish
